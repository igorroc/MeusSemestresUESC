...
def run():
    try:
        with open("entradas/euler.txt", "r") as file:
        ...
        print("O arquivo euler.txt não foi encontrado.")
        exit(1)
    except (ValueError, IndexError):
        print("O arquivo euler.txt não está formatado corretamente.")
        exit(1)

    f = lambda x, y: eval(equation)
    solution = euler_method(f, x0, y0, h, iterations)

    with open("saidas/euler.txt", "w") as file:
